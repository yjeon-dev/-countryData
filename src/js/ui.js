...
const ui = {
  ...
  addLangToElement: function (element, language) {
    let lang_code = util.langCodeForLanguage(language);
    if (lang_code) {
      element.setAttribute('lang', lang_code);
    } else {
      element.setAttribute('lang', 'unknown');
    }
  ...
  countryCodeToFlagUrl: function (countryCode, size) {
    if ([16, 24, 32, 48, 64].includes(size)) {
      //let url = `https://www.countryflags.io/${countryCode}/flat/${size}.png`;
      var lower = countryCode.toLowerCase();
      let url = `https://flagcdn.com/40x30/${lower}.png`;
      return url;
    }
  ...
  countryCodeToImg: function (countryCode) {
    let href = ui.countryCodeToFlagUrl(countryCode, 24);
    if (href) {
  ...
  countryToRow: function (country, language) {
    ...
    if (country.code === '') {
      ...
      let flag_td = document.createElement('td');
      flag_td.textContent = flag;
      row.appendChild(flag_td);
    } else {
      let flag = ui.countryCodeToImg(country.code, 24);

      let flag_td = document.createElement('td');
      flag_td.appendChild(flag);
      row.appendChild(flag_td);
    }

    let code_td = document.createElement('td');
    code_td.textContent = country.code;
    row.appendChild(code_td);

    let country_td = document.createElement('td');
    country_td.textContent = country.name;
    country_td = ui.addLangToElement(country_td, language);

    row.appendChild(country_td);
    ...
    area.textContent = country.areaInKm2;
    row.appendChild(area);

    let pop = document.createElement('td');
    pop.textContent = country.population;
    row.appendChild(pop);
  ...
  countriesToTable: function (countries, language) {
    ...
    for (let country of countries) {
      let row = ui.countryToRow(country, language);
